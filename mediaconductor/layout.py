"""The one map of where MediaConductor puts things on disk.

Two roots, and only two:

``<workspace>/data/``
    Everything MediaConductor downloads or generates: source chapters,
...
``<install>/runtime/``
    ...
    re-download — and re-creatable with ``mediaconductor setup``.

``bgm/`` and ``vocal/`` sit beside both and belong to the user: licensed
music and narrator reference takes that MediaConductor reads and never
...
stay stdlib-only and every ``mediaconductor`` import is function-local:
...
def data_root() -> Path:
    ...
    ``MEDIACONDUCTOR_DATA_ROOT`` relocates it wholesale (a second drive, a
    ...
    """
    configured = os.environ.get("MEDIACONDUCTOR_DATA_ROOT")
    if configured:
...
def runtime_root() -> Path:
    ...
    per install and survive a ``data/`` wipe. ``MEDIACONDUCTOR_HOME``
...
def library_root() -> Path:
    ...
    """
    from mediaconductor.path_safety import validate_relative_subpath
...
DATA_README = """# MediaConductor data

Everything MediaConductor downloaded or generated lives here, and nothing
else does.
...
`mediaconductor workspace-reset` — same result, but it refuses to run while
