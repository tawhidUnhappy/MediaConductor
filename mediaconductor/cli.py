"""mediaconductor.cli — the single ``mediaconductor`` entry point.

Every tool in the package is reachable as a subcommand::

    mediaconductor <command> [args...]
    mediaconductor <command> --help
    mediaconductor --help
    mediaconductor --version

Subcommand modules are imported lazily, so ``mediaconductor --help`` stays fast and
...
from mediaconductor.brand import CLI_NAME, LEGACY_CLI_NAME, PRODUCT_NAME, mirror_legacy_environment
from mediaconductor.tools.vendored import ensure_vendored_path

# Legacy MANGAEASY_* configuration keeps working: mirror it onto the new
# MEDIACONDUCTOR_* names before any module reads its environment.
...
# see mediaconductor/tools/vendored.py. Pure filesystem check, no network access,
...
COMMANDS: dict[str, tuple[str, str, str, str]] = {
    ...
    "workspace-layout":     ("mediaconductor.workspace",                            "main",        "Setup",            "Report every resolved persistent root and whether it stays inside data/ (production) or runtime/ (tools) (--json)."),
    "workspace-reset":      ("mediaconductor.workspace",                            "reset_main",  "Setup",            "Start fresh: delete everything downloaded/generated under data/ (dry run unless --confirm). Tools, caches, tokens, bgm/ and vocal/ survive."),
    ...
    "mcp":                  ("mediaconductor.mcp_server",                           "main",        "Setup",            "Run an MCP stdio server exposing MediaConductor as typed tools for AI assistants."),
    ...
    "video-join":           ("mediaconductor.video_pipeline.make_long_video",       "main",        "Video pipeline",   "Join item videos into one long video (optional BGM)."),
    ...
    "video-clean-audio":    ("mediaconductor.video_pipeline.cleanup_audio",         "main",        "Video pipeline",   "Clear generated audio for selected items (archived, not lost -- see audio-takes-list)."),
    "video-clean-video":    ("mediaconductor.video_pipeline.cleanup_videos",        "main",        "Video pipeline",   "Delete rendered item videos."),
    "video-clean-work":     ("mediaconductor.video_pipeline.cleanup_work",          "main",        "Video pipeline",   "Delete the work/ scratch directory."),
    "video-clean-all":      ("mediaconductor.video_pipeline.cleanup_all",           "main",        "Video pipeline",   "Delete ALL generated output for a project (audio, videos, archives) in one go -- source chapters are untouched."),
    "audio-takes-list":     ("mediaconductor.video_pipeline.audio_takes",           "list_main",   "Video pipeline",   "List previously archived audio takes (old/run_NNNN/) for a project."),
    "audio-takes-restore":  ("mediaconductor.video_pipeline.audio_takes",           "restore_main","Video pipeline",   "Restore an archived audio take as the active audio instead of regenerating it."),

    # ── YouTube ───────────────────────────────────────────────────────────────
    "youtube-profiles":     ("mediaconductor.youtube.auth",                         "profiles_main","YouTube",         "List isolated YouTube account profiles and cached channels (--json)."),
    "youtube-auth":         ("mediaconductor.youtube.auth",                         "auth_main",   "YouTube",          "Connect a named YouTube account profile (browser consent; default profile: default)."),
    "youtube-status":       ("mediaconductor.youtube.auth",                         "status_main", "YouTube",          "Show one YouTube profile's status (--json); --verify checks it live."),
    "youtube-logout":       ("mediaconductor.youtube.auth",                         "logout_main", "YouTube",          "Disconnect one YouTube profile (delete only that profile's token)."),
    "youtube-upload":       ("mediaconductor.youtube.upload",                       "main",        "YouTube",          "Upload through a selected account profile (resumable; default privacy: private)."),
    "youtube-list":         ("mediaconductor.youtube.list_videos",                  "main",        "YouTube",          "List a selected profile's uploads (id, title, privacy, date)."),
    ...
    "gutter-split":         ("mediaconductor.panels.gutter",                        "main",        "Manga: acquire",   "Split pages along gutters into panels (low-level engine)."),
    "webtoon-split":        ("mediaconductor.panels.webtoon",                       "main",        "Manga: acquire",   "Split webtoon items into panels with auto-split, gap rescue and verify sheets."),
    "webtoon-cutcheck":     ("mediaconductor.panels.cutcheck",                      "main",        "Manga: acquire",   "Render full-res review windows around every forced cut / short panel (crop QA)."),
...
def _print_help(stream=None, mode: str | None = None, all_commands: bool = False) -> None:
    ...
    from mediaconductor.modes import MODES, normalize_mode
...
def commands_main() -> int:
    """`mediaconductor commands [--json] [--full]` — the machine-readable catalog.
    ...
    schema from mediaconductor/command_spec.py — flags, types, required — so an
    ...
    from mediaconductor.modes import MODES
    ...
    if args.full:
        from mediaconductor.command_spec import LONG_RUNNING, cli_args_schema
...
def _dispatch(command: str, rest: list[str]) -> int:
    ...
    try:
        ...
        from mediaconductor.config import ConfigError
        from mediaconductor.path_safety import UnsafePathComponentError
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not argv or argv[0] in ("-h", "--help", "help"):
        # `mediaconductor help <command>` -> show that command's own help.
