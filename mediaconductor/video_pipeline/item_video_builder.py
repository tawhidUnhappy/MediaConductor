...
from mediaconductor.video_pipeline.common import item_dirs, merge_item_selection, project_name
...
from mediaconductor.video_pipeline.blur_background import (
...
def build_item_videos(config: VideoBuildConfig) -> Path:
    ...
    total = len(items)
    print(f"MEDIACONDUCTOR_PROGRESS 0/{total} Rendering videos", flush=True)
    if config.workers == 1 or total <= 1:
        for idx, item_dir in enumerate(items, start=1):
            build_one_chapter(item_dir, config)
            print(f"MEDIACONDUCTOR_PROGRESS {idx}/{total} Rendered {item_dir.name}", flush=True)
    else:
        ...
        def _render(item_dir: Path) -> None:
            ...
            with done_lock:
                done_count += 1
                print(f"MEDIACONDUCTOR_PROGRESS {done_count}/{total} Rendered {item_dir.name}", flush=True)
