...
def clamp_gpu_workers(requested: int) -> int:
    ...
    MEDIACONDUCTOR_UNSAFE_GPU_WORKERS=1 opts out for hardware that has actually
    been tested higher.
    """
    import sys
    ...
    if os.environ.get("MEDIACONDUCTOR_UNSAFE_GPU_WORKERS") == "1":
        print(f"[warn] --gpu-workers {requested} exceeds the tested-safe maximum "
              f"{GPU_WORKERS_SAFE_MAX}; proceeding because MEDIACONDUCTOR_UNSAFE_GPU_WORKERS=1.",
              file=sys.stderr)
        return requested
    print(f"[warn] --gpu-workers {requested} clamped to {GPU_WORKERS_SAFE_MAX}: more "
          ...
          f"(set MEDIACONDUCTOR_UNSAFE_GPU_WORKERS=1 to override on tested hardware).",
          file=sys.stderr)
