"""mediaconductor.panels.webtoon
Item-pipeline webtoon splitter with verification output (`mediaconductor webtoon-split`).

Builds on mediaconductor.panels.gutter (same detection code path as `gutter-split`)
...
from mediaconductor.panels.gutter import (
    GutterConfig,
    _recursive_ranges,
    collect_image_paths,
    load_gutter_config,
...
from mediaconductor.utils import emit_result, next_archive_run_dir
...
def write_ranges_manifest(
    ...
    *,
    ...
    Don't compute merge indices by hand — ``mediaconductor webtoon-override``
    ...
    manifest = {
        ...
        "merge_note": "merge indices are 0-based positions in `base` (the "
                      ...
                      "`mediaconductor webtoon-override` to compute them",
        "final": [
...
def parse_args() -> argparse.Namespace:
    from mediaconductor.path_safety import portable_prefix_template_arg, relative_subpath_arg
