...
def _resolve_device(pref: str = "auto") -> str:
    ...
    if pref == "auto":
        if torch is None:
            return "cpu"
        from mediaconductor.tools.external import resolve_device
...
USE_EXTERNAL_MAGI = os.environ.get("MEDIACONDUCTOR_EXTERNAL_MAGI", "1") != "0"
...
def _run_magi_external(image_path: Path) -> Dict:
    ...
    try:
        ...
        proc = runtime.run(cmd, cwd=MAGI_V3_DIR, env=tool_env(), capture_output=True, text=True, check=False)
...
def _manga_reading_order(
    ...
    if rtl is None:
        try:
            from mediaconductor.config import load_system_config
...
if __name__ == "__main__":
    ...
    if len(_sys.argv) < 2:
        print("Usage: python -m mediaconductor.panels.ai <image_path>")
        _sys.exit(1)
