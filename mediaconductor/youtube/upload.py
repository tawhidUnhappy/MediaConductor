"""`mediaconductor youtube-upload` — resumable upload to the connected channel.
...
Machine contract (docs/ai-guide.md): MEDIACONDUCTOR_PROGRESS lines per chunk and
a final ``MEDIACONDUCTOR_RESULT {"video_id": ..., "url": ...}`` on success.
    ...
    with video.open("rb") as f:
        while offset < total:
            ...
            if response.status_code == 308:
                ...
                print(f"MEDIACONDUCTOR_PROGRESS {total}/{total} uploading", flush=True)
            ...
            print(f"MEDIACONDUCTOR_PROGRESS {offset}/{total} uploading", flush=True)
...
def main() -> int:
    ...
    from mediaconductor.youtube.auth import (
