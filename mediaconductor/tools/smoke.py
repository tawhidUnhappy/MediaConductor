...
SMOKE_REVIEWER = "smoke-test (synthetic fixture generated by this run)"


def record_fixture_reviews(project_root: Path) -> None:
    """Approve the fixture's own generated bytes so the real gates apply.

    ``source_subdir="panels"`` because the fixture has no ``download/`` stage:
...
def main() -> int:
    ...
    # TTS and rendering are gated on hash-bound crop/narration reviews, and
    # there is deliberately no bypass flag. The fixture is generated by this
    # function a few lines above, so recording a review over it is a truthful
    # statement about known bytes, not an assertion about unseen artwork — and
    # it means the smoke test exercises the real review path rather than
    # routing around it.
    record_fixture_reviews(project_root)
    checks["review"] = "recorded (synthetic fixture)"
