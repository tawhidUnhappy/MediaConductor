...
def _default_frozen_root() -> Path:
    """Writable data root for a frozen build running without MEDIACONDUCTOR_ROOT.

    This matters for a standalone frozen CLI when the caller hasn't set
    MEDIACONDUCTOR_ROOT. The exe's own folder is not reliably writable everywhere:
...
def app_root() -> Path:
    ...
    Dev checkout: the repo root (parent of the ``mediaconductor`` package).
    An explicit MEDIACONDUCTOR_ROOT env var always wins when present.
    """
    configured = os.environ.get("MEDIACONDUCTOR_ROOT")
    if configured:
...
def _share_caches() -> bool:
    ...
    return os.environ.get("MEDIACONDUCTOR_SHARE_CACHES", "").strip().lower() in (
        "1", "true", "yes", "on",
    )
...
def tool_env(base: dict[str, str] | None = None) -> dict[str, str]:
    ...
    Set ``MEDIACONDUCTOR_SHARE_CACHES=1`` to deliberately defer to those inherited
    locations instead (a shared cross-project cache); then they are only
    filled in when absent.

    Always drops VIRTUAL_ENV/PYTHONHOME inherited from mediaconductor's own
    ...
    it's set. Left inherited, that silently installs into mediaconductor's own
...
def run_tool_python(tool_name: str, script: Path, args: list[str], *, env_var: str | None = None) -> None:
    ...
    runtime.run(command, cwd=tool_dir, env=tool_env(), check=True)
...
def main() -> int:
    ...
    print("Override with MEDIACONDUCTOR_TOOLS_DIR or per-tool env vars above.")
    return 0


def where_main() -> int:
    """`mediaconductor where [--json]` — resolved paths + environment facts.
