"""mediaconductor.images.ai_zip_cli — CLI wrapper for panels_to_ai_zip.

Exposed as `mediaconductor ai-zip`. Lets non-Python callers (scripts, agents)
...
from mediaconductor.images.ai_zip import panels_to_ai_zip
...
def main() -> int:
    ...
    def progress(value: int, total: int, label: str) -> None:
        print(f"MEDIACONDUCTOR_PROGRESS {value}/{total} {label}", flush=True)
