"""mediaconductor.images.thumbnail_compose — compose a thumbnail from panel art.

``mediaconductor thumbnail-compose`` turns **manga panels the crop review
...
def bundled_font(name: str = "edosz.ttf") -> str | None:
    """Path to a font shipped with the package, or None when missing."""
    candidate = Path(__file__).resolve().parent.parent / "assets" / "fonts" / name
