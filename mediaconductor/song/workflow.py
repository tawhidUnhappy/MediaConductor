...
from mediaconductor.utils import archive_before_overwrite, atomic_write_json, emit_result, parse_result_marker
from mediaconductor.video_pipeline.ffmpeg_tools import choose_h264_encoder, h264_encoder_args
from mediaconductor.youtube.store import validate_profile
...
def _run_streaming(argv: list[str], *, accepted_codes: tuple[int, ...] = (0,)) -> dict:
    ...
    for line in process.stdout:
        ...
        payload = parse_result_marker(line)
...
def render_video(background: Path, audio: Path, subtitles: Path, output: Path,
    ...
    runtime.run(command, check=True)
