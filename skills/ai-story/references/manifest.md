...
Z-Image Turbo runs at `guidance_scale 0.0`, so `production.negative_prompt` is not a true CFG negative: the builder appends it to the prompt as a literal `Avoid: ...` instruction. Keep it the short generated list of abstract failure modes. Never add concrete imagery nouns you do not want drawn (for example `comic panel grid` or `second moon`) — at guidance 0 the encoder reads those nouns as content and becomes *more* likely to draw them. Fix an unwanted element by strengthening the shot instruction or the relevant card, not by growing the Avoid list.
