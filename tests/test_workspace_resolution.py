...
import mangaeasy.config as config
...
def test_env_var_wins(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setenv("MANGAEASY_PROJECT_ROOT", str(workspace))
...
def test_cwd_with_config_json_wins_over_registration(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.delenv("MANGAEASY_PROJECT_ROOT", raising=False)
    monkeypatch.setenv("MANGAEASY_HOME", str(tmp_path / "data"))
...
def _make_fresh_clone(path: Path) -> Path:
    ...
    (path / "mangaeasy").mkdir(parents=True, exist_ok=True)
...
def test_an_arbitrary_directory_is_still_not_a_workspace(tmp_path: Path):
    ...
    # A pyproject alone is some other project, not a mangaEasy checkout.
