...
The self-contained promise ("everything MediaConductor writes lives in one
folder") only holds if a tool subprocess's HF/torch/uv caches land under
`runtime/cache/`, even on a machine that exports a global `HF_HOME`/
`UV_CACHE_DIR` for other tools. `tool_env()` force-pins them for exactly
that reason; `MEDIACONDUCTOR_SHARE_CACHES=1` is the opt-out.
"""

import os

from mediaconductor.tools.external import tool_env

CACHE_VARS = ("HF_HOME", "HF_HUB_CACHE", "TRANSFORMERS_CACHE", "TORCH_HOME", "UV_CACHE_DIR")


def test_forces_caches_under_runtime_over_inherited_globals(tmp_path, monkeypatch):
    monkeypatch.setenv("MEDIACONDUCTOR_HOME", str(tmp_path / "runtime"))
    monkeypatch.delenv("MEDIACONDUCTOR_SHARE_CACHES", raising=False)
...
def test_share_caches_opt_in_defers_to_inherited(tmp_path, monkeypatch):
    monkeypatch.setenv("MEDIACONDUCTOR_HOME", str(tmp_path / "runtime"))
    monkeypatch.setenv("MEDIACONDUCTOR_SHARE_CACHES", "1")
    monkeypatch.setenv("HF_HOME", r"D:\shared\hf")

    env = tool_env()
    assert env["HF_HOME"] == r"D:\shared\hf"
    # Vars the ambient env does NOT set still fall back under runtime/.
    monkeypatch.delenv("UV_CACHE_DIR", raising=False)
    assert str(tmp_path / "runtime") in tool_env()["UV_CACHE_DIR"]
...
def test_share_caches_accepts_truthy_spellings(tmp_path, monkeypatch):
    ...
    for val in ("1", "true", "YES", "On"):
        monkeypatch.setenv("MEDIACONDUCTOR_SHARE_CACHES", val)
        assert tool_env()["HF_HOME"] == r"D:\shared\hf", val
    monkeypatch.setenv("MEDIACONDUCTOR_SHARE_CACHES", "0")
