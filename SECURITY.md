...
- Keep project and output roots in a dedicated workspace. `mediaconductor workspace-layout --json` reports every resolved persistent root; `doctor` warns when one escapes. Destructive cleanup requires a strict allowed root and exact target-name confirmation, and never touches `library/`.
