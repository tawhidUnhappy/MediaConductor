...
Security fixes target the latest release on `main`. The previous `mediaconductor` command is a compatibility alias, not a separate security surface.
...
- Start MCP with a dedicated, repeatable `--allow-root` workspace (or accept its startup-directory default). The policy covers direct paths, nested typed jobs, configured media, and the review/final-video paths; it is a same-user stdio guardrail, not an OS sandbox.
- Keep project and output roots in a dedicated workspace. `mangaeasy workspace-layout --json` reports every resolved persistent root; `doctor` warns when one escapes. Destructive cleanup requires a strict allowed root and exact target-name confirmation, and never touches `data/library/`.
...
mangaEasy includes no manga, licensed music, or voice-cloning sample. Clearing the source material, the music, and the narrator voice for use is the operator's responsibility; the tool does not verify it.
