...
Generation provenance is stored in `review/reference_generation.json`, `review/scene_generation.json`, and `review/video_generation.json`. Contract digests detect authored-input changes; content hashes detect regeneration or file replacement. The video contract additionally records FPS, requested and resolved TTS, narration and speaker fields, and the content hash of an IndexTTS speaker WAV. Keep that voice reference available and unchanged until publication. The builder archives stale images, forces stale narration/video inputs to rebuild, and blocks each downstream stage until the exact new artifacts are reviewed.
