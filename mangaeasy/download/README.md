...
- [`mangadex.py`](mangadex.py) (`mangaeasy download`) — download a chapter (or
  a batch: `--chapters 0-12 14 20.5`) from MangaDex. Writes pages to the item's
  `download/` folder and **maintains `library/<Project>/manga.json`** — the
  per-project source record (source site, canonical MangaDex title URL, the
  original pasted URL, fetched title, and a per-chapter map of
