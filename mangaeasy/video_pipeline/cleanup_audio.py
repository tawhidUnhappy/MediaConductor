...
from mangaeasy.utils import LazyArchiveRunDir, remove_tree
...
def main() -> int:
    ...
    if args.purge:
        for audio_path, _, _ in targets:
            remove_tree(audio_path) if audio_path.is_dir() else audio_path.unlink()
        print("\nPermanently deleted.")
        return 0
    ...
    for audio_path, _, label in targets:
        ...
        for child in audio_path.iterdir():
            ...
            if child_destination.exists():
                remove_tree(child_destination) if child_destination.is_dir() else child_destination.unlink()
