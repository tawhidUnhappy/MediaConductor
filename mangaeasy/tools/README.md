...
| [`hardware.py`](hardware.py) | — | GPU detection used to choose narration and panel-analysis engines |
...
`batch_detect_magi.py` (used by `page-split`).
...
- Model-specific facts that must not be "optimized" away (including MAGI's
  Transformers version and eager-attention requirement) live in
