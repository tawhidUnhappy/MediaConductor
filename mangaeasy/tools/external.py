...
def app_root() -> Path:
    """Directory this mangaEasy install keeps its data in.
...
def data_home() -> Path:
    ...
    ``<app_root>/runtime`` — see :mod:`mangaeasy.layout` for why it sits
    ...
    MANGAEASY_HOME to share one tool tree across checkouts.
    """
    from mangaeasy.layout import runtime_root
...
def tools_home() -> Path:
    ...
    Default `<runtime>/tools`. Override with MANGAEASY_TOOLS_DIR.
    """
    from mangaeasy.layout import tools_root
...
def tool_env(base: dict[str, str] | None = None) -> dict[str, str]:
    ...
    this install's own `runtime/cache/` dir, so the "everything mangaEasy
...
def where_main() -> int:
    ...
    info = {
        ...
        # workspace, set MANGAEASY_PROJECT_ROOT, or re-run `setup` there.
        ...
        "env_overrides": {
            ...
            for name in ("MANGAEASY_ROOT", "MANGAEASY_HOME",
