"""`mangaeasy mcp` — the manga-production MCP stdio server.

Exposes mangaEasy as typed tools any MCP-capable AI assistant can call.
...
`mangaeasy` subcommand (via `runtime.cli_command`), so the lazy-import
design and process isolation are untouched.

Tool schemas come from mangaeasy/command_spec.py — the single declarative
table shared with `mangaeasy commands --json --full`. Add/change tools
there, not here.
...
`claude mcp add mangaeasy -- uv --project <repo> run mangaeasy mcp
...
from mangaeasy.brand import CLI_NAME, MCP_SERVER_NAME, PRODUCT_NAME, RESULT_MARKERS
from mangaeasy.command_spec import JSON_COMMANDS, LONG_RUNNING, TOOLS
from mangaeasy.modes import DEFAULT_MODE, MODES, normalize_mode
...
def main() -> int:
    parser = argparse.ArgumentParser(
        prog=f"{CLI_NAME} mcp",
        description="Run the mangaEasy MCP stdio server with a mode-scoped tool catalog.",
    )
