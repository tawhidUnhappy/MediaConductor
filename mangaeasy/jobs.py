...
nvidia-smi" was the documented workaround. This module replaces that:
    ...
    mangaeasy job-start --tool run_full_pipeline --arguments-json '{...}'
    -> {"ok": true, "job_id": "20260714-153000-video-a1b2c3d4", ...}   (returns instantly)
    mangaeasy job-status 20260714-153000-video-a1b2c3d4 --json
...
def _scan_log_markers(log_path: Path) -> tuple[str | None, dict | None]:
    ...
    external envs still print the legacy MEDIACONDUCTOR_* one.
    """
    progress = None
    result = None
    try:
...
def start_main() -> int:
    ...
    from mangaeasy.mcp_server import _allowed_tools, _build_args
    from mangaeasy.modes import DEFAULT_MODE
