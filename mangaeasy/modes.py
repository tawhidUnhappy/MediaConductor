"""Single manga-production catalog shared by the CLI, MCP server, and setup."""
...
COMMON_TOOLS = frozenset({
    "modes", "setup", "doctor", "where", "workspace_layout", "install_tool",
...
MANGA_COMMANDS = frozenset({
    ...
    "panel-reading-sheets", "narration-review-sheets", "narration-edit", "video-validate",
    "video-chapters", "video-quality", "video-audio-audit", "video-fade-audio",
...
MANGA_TOOLS = frozenset({
    ...
    "run_full_pipeline", "manga_review", "panel_decisions",
...
MODES: dict[str, ModeSpec] = {
    "manga-video": ModeSpec(
        ...
        optional_external_tools=("index-tts", "magi-v3", "deepseek-ocr2"),
    ),
}


DEFAULT_MODE = "manga-video"
...
def normalize_mode(value: str | None) -> str | None:
    ...
    aliases = {"manga": "manga-video", "manhwa": "manga-video", "webtoon": "manga-video"}
