...
def looks_like_workspace(root: Path) -> bool:
    ...
    * a source checkout of mangaEasy itself (``pyproject.toml`` beside
      the ``mangaeasy`` package), which is where a dev install lives.
    """
    root = Path(root)
    ...
    if (root / "data").is_dir():
        return True
    return (root / "pyproject.toml").is_file() and (root / "mangaeasy").is_dir()
...
def _project_root() -> Path:
    ...
    1. ``MANGAEASY_PROJECT_ROOT`` — explicit always wins.
    2. The cwd, when it actually is a workspace (see
       :func:`looks_like_workspace`).
    3. The workspace registered by ``mangaeasy setup`` (workspace.json).
...
def hf_cache_dir() -> Path:
    ...
    not a constant, so MANGAEASY_HOME still moves it.
    """
